...
public class SellersServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		purchase.setEditCompany(userBean.isEditCompanies());
...
//		Purchase.setErrorMessage(buttonName);
		...
		case "submit_add":
			if(!purchase.isAddCompany() && purchase.getCompany()==null)
				...
				purchase.save(request, userBean);
			break;
		case "submit_cancel":
			purchase.cancelEdit();
			break;
	...
	private boolean moveComponents(HttpServletRequest request) {
		...
		if (cm.getFrom().equals(ComponentsMovement.getPlace(cm, HTTP_ADDRESS))) {
			...
			error.setErrorMessage("You did not finish move from "+cm.getFrom().getName()+". <small>(E059)</small>");
